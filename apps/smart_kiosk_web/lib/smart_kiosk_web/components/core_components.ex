defmodule SmartKioskWeb.CoreComponents do
  @moduledoc """
        ...
        <div class="flex min-h-full items-center justify-center">
          <div class="w-full max-w-3xl p-4 sm:p-6 lg:py-8">
            <.focus_wrap
              ...
              class="relative hidden rounded-3xl bg-[#0B0F1A] border border-white/10 p-14 shadow-2xl shadow-violet-500/10 transition-all"
            >
              <div class="absolute top-6 right-5">
                <button
                  type="button"
                  phx-click={JS.exec("data-cancel", to: "##{@id}")}
                  class="flex-none p-3 opacity-20 hover:opacity-40 transition-opacity"
  ...
  @doc """
  Renders a small back link component with an arrow icon.

  ## Examples

      <.back navigate="/posts">Back</.back>
      <.back href="/">Home</.back>
  """
  ...
  def back(assigns) do
    assigns =
      assign_new(assigns, :class, fn ->
        [
          "inline-flex items-center gap-1 text-sm text-slate-400 hover:text-slate-200 transition-colors"
        ]
      end)
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(SmartKioskWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(SmartKioskWeb.Gettext, "errors", msg, opts)
    end
  end
