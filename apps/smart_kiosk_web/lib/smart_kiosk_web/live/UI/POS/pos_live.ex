  ...
  <div class="flex h-[calc(100vh-64px)] overflow-hidden relative">
    ...
    <aside class={[
      ...
      <div class="p-3 sm:p-4">
        ...
        <h2 class="hidden lg:block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Menu</h2>

        <nav class="space-y-0.5">
          <button type="button" phx-click="switch_tab" phx-value-tab="products"
            class={[
          ...
          <button type="button" phx-click="switch_tab" phx-value-tab="expenses"
            class={[
              "w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl transition-all text-sm",
          ...
          <button type="button" phx-click="switch_tab" phx-value-tab="customers"
            class={[
              "w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl transition-all text-sm",
          ...
          <button type="button" phx-click="switch_tab" phx-value-tab="orders"
            class={[
              "w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl transition-all text-sm",
          ...
          <button type="button" phx-click="switch_tab" phx-value-tab="revenue"
            class={[
              "w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl transition-all text-sm",
          ...
          <button type="button" phx-click="switch_tab" phx-value-tab="sales"
            class={[
              "w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl transition-all text-sm",
          ...
          <button type="button" phx-click="switch_tab" phx-value-tab="settings"
            class={[
              "w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl transition-all text-sm",
      ...
      <div class="mx-3 mt-2 p-3 bg-violet-500/10 border border-violet-500/20 rounded-xl">
        ...
        <div class="mt-2 pt-2 border-t border-violet-500/20 flex justify-between items-center">
          ...
          <span class="text-sm font-bold text-emerald-400">KES <%= Decimal.to_string(@pos_revenue_today, :normal) %></span>
        </div>
      </div>
    </aside>
    ...
    <main class="flex-1 flex flex-col overflow-hidden">
      <header class="px-2 py-2 sm:px-4 sm:py-3 border-b border-white/10 flex items-center justify-between gap-1 flex-shrink-0">
        <div class="flex items-center gap-1 min-w-0">
          <button phx-click="toggle_mobile_menu" class="lg:hidden p-1.5 text-slate-400 hover:text-white flex-shrink-0">
        ...
        <div class="flex items-center gap-1 flex-shrink-0">
          ...
          <% end %>
          <button phx-click="toggle_mobile_cart" class="lg:hidden p-1.5 text-slate-400 hover:text-white relative">
            <.icon name="hero-shopping-cart" class="w-5 h-5" />
            <%= if length(@cart) > 0 do %>
              <span class="absolute -top-1 -right-1 w-4 h-4 bg-violet-500 rounded-full text-[9px] font-bold flex items-center justify-center"><%= length(@cart) %></span>
            <% end %>
          </button>
        </div>
      </header>
              ...
              <%= for product <- @products do %>
                <button type="button"
                  ...
                  class={[
                    ...
                    product.stock_qty > 0 && product.stock_qty <= product.low_stock_threshold && "border-amber-500/40",
                    product.stock_qty > product.low_stock_threshold && "border-white/10 hover:border-violet-500/30"
                  ]}>
                  ...
                  <%= if product.stock_qty > 0 && product.stock_qty <= product.low_stock_threshold do %>
                    <div class="absolute top-1 left-1 z-10 px-1 py-0.5 bg-amber-500/90 rounded text-[7px] font-bold text-black uppercase">Low</div>
                  <% end %>
                  <div class={[
                    ...
                    <img :if={Enum.any?(product.images)} src={List.first(product.images).url} class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                    <.icon :if={Enum.empty?(product.images)} name="hero-shopping-bag" class="w-6 h-6 sm:w-8 sm:h-8 text-slate-600" />
                  </div>
                  <h3 class="font-semibold text-[11px] sm:text-xs truncate text-slate-200"><%= product.name %></h3>
                  <div class="flex items-center justify-between mt-0.5">
                    <p class="text-violet-400 font-bold text-[10px] sm:text-xs">KES <%= product.price %></p>
                    <span class={[
                      "text-[8px] uppercase font-semibold",
                      product.stock_qty == 0 && "text-red-400",
                      product.stock_qty > 0 && product.stock_qty <= product.low_stock_threshold && "text-amber-400",
            ...
            <div class="space-y-3">
              <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
                <div class="bg-white/5 border border-white/10 rounded-xl p-3 flex items-center gap-3">
                  ...
                  <div class="min-w-0">
                    <p class="text-[10px] text-slate-500 uppercase tracking-wider">Revenue Today</p>
                    <p class="text-lg font-bold text-white">KES <%= Decimal.to_string(@pos_revenue_today, :normal) %></p>
                  </div>
                </div>
                <div class="bg-white/5 border border-white/10 rounded-xl p-3 flex items-center gap-3">
                  ...
                  <div class="min-w-0">
                    ...
                    <p class="text-lg font-bold text-white">KES <%= Decimal.to_string(@my_stats_today.total, :normal) %></p>
                ...
                <div class="bg-white/5 border border-white/10 rounded-xl p-3 flex items-center gap-3">
                  ...
                  <div class="min-w-0">
                    <p class="text-[10px] text-slate-500 uppercase tracking-wider">Transactions</p>
              ...
              <div class="grid grid-cols-1 lg:grid-cols-2 gap-3">
                <div class="bg-white/5 border border-white/10 rounded-xl p-3 overflow-x-auto">
                  ...
                  <table class="w-full text-xs">
                    <thead><tr class="text-slate-500 uppercase tracking-wider border-b border-white/10">
                    ...
                    <tbody class="divide-y divide-white/5">
                      <%= for c <- @pos_cashier_sales_today do %>
                        <tr class={if c.user_id == @user.id, do: "bg-violet-500/5 font-semibold"}>
                          ...
                          <td class="text-right font-mono text-emerald-400">KES <%= Decimal.to_string(c.total, :normal) %></td>
                        </tr>
                      <% end %>
                    </tbody>
                  </table>
                  ...
                  <div class="space-y-1.5">
                    <%= for order <- @pos_orders_today |> Enum.take(10) do %>
                      <div class="flex items-center justify-between p-2 bg-white/5 border border-white/10 rounded-lg text-xs">
                        <span class="font-mono font-bold text-white">POS-<%= String.slice(order.id, 0, 6) |> String.upcase() %></span>
                        <span class="font-mono text-white">KES <%= Decimal.to_string(order.total, :normal) %></span>
                        <button
                          type="button"
          ...
          <% true -> %>
            <div class="flex-1 flex items-center justify-center">
              <div class="text-center">
                ...
                <h2 class="text-lg font-semibold text-slate-400"><%= String.capitalize(@active_tab) %></h2>
    ...
    <section class={[
      ...
      <div class="px-3 py-2 sm:px-4 sm:py-3 border-b border-white/10 flex justify-between items-center flex-shrink-0">
        ...
        <div class="flex items-center gap-2">
          <button phx-click="toggle_mobile_cart" class="lg:hidden text-slate-400 text-xs">Close</button>
          <button phx-click="clear_cart" class="text-red-400 text-xs hover:text-red-300">Clear</button>
        </div>
      </div>

      <%= if @receipt do %>
        <div class="flex-1 overflow-y-auto p-3 sm:p-4">
          <div id="receipt-container" phx-hook="PrintReceipt" class="digital-receipt bg-white text-black rounded-lg p-3 text-xs font-mono">
            <div class="font-bold text-sm"><%= @receipt.shop.name %></div>
            <div class="text-[10px] text-gray-500"><%= @receipt.inserted_at |> DateTime.to_string() %></div>
            ...
            <div class="divide-y border-t border-gray-200 mt-2">
              <%= for item <- @receipt.items do %>
                <div class="flex justify-between py-1">
                  ...
                  <span class="font-mono flex-shrink-0">KES <%= :erlang.float_to_binary(Decimal.to_float(item.product.price) * item.quantity, decimals: 2) %></span>
                </div>
              <% end %>
            </div>
            ...
            <%= if @receipt.method == "cash" do %>
              <div class="flex justify-between text-[10px] text-gray-500"><span>Received</span><span>KES <%= :erlang.float_to_binary(@receipt.received, decimals: 2) %></span></div>
              <div class="flex justify-between text-[10px] font-semibold"><span>Change</span><span>KES <%= :erlang.float_to_binary(@receipt.change, decimals: 2) %></span></div>
            <% end %>
            <div class="text-center text-[10px] text-gray-400 mt-2 pt-1 border-t border-gray-100">Thank you!</div>
          </div>
          <div class="grid grid-cols-3 gap-1.5 mt-2 no-print">
            <button phx-click="print_receipt" phx-value-order-id={@receipt.id} class="py-2 bg-slate-800 hover:bg-slate-700 text-xs font-semibold rounded-lg border border-white/10">🖨 Print</button>
            <button phx-click="share_receipt" class="py-2 bg-slate-800 hover:bg-slate-700 text-xs font-semibold rounded-lg border border-white/10">↗ Share</button>
            <button phx-click="new_order" class="py-2 bg-violet-600 hover:bg-violet-500 text-xs font-semibold rounded-lg">+ New</button>
          </div>
        </div>
      <% else %>
        <%= if @show_payment do %>
          <div class="flex-1 overflow-y-auto p-3 sm:p-4 space-y-4">
            <button phx-click="hide_payment" class="text-violet-400 text-xs font-semibold flex items-center gap-1">
              <.icon name="hero-arrow-left" class="w-3 h-3" /> Back
            </button>
            <div class="bg-white/5 border border-white/10 rounded-xl p-4 text-center">
              <p class="text-sm text-slate-400">Amount Due</p>
              <p class="text-2xl font-bold text-violet-400">KES <%= :erlang.float_to_binary(@cart_total, decimals: 2) %></p>
            </div>
            ...
            <div class="bg-white/5 border border-white/10 rounded-xl p-3">
              ...
              <form phx-change="update_cash" class="flex gap-2 items-center">
                <input name="cash_received" type="text" inputmode="numeric"
                ...
                <span class="text-xs text-slate-400">Change: <span class="font-bold text-white"><%= :erlang.float_to_binary(@cash_change, decimals: 2) %></span></span>
              </form>
              <button phx-click="complete_payment" phx-value-method="cash"
            ...
            <button phx-click="show_mpesa_modal" class="w-full p-3 bg-white/5 border border-white/10 hover:border-yellow-500/50 rounded-xl flex items-center justify-between group">
              <span class="flex items-center gap-2 text-sm font-bold">
                <.icon name="hero-phone" class="w-4 h-4 text-yellow-500" /> M-PESA
              </span>
              <.icon name="hero-chevron-right" class="w-4 h-4 text-slate-600 group-hover:text-yellow-500" />
            </button>

            <button phx-click="show_card_modal" class="w-full p-3 bg-white/5 border border-white/10 hover:border-blue-500/50 rounded-xl flex items-center justify-between group">
              <span class="flex items-center gap-2 text-sm font-bold">
                <.icon name="hero-credit-card" class="w-4 h-4 text-blue-500" /> Card
              </span>
              <.icon name="hero-chevron-right" class="w-4 h-4 text-slate-600 group-hover:text-blue-500" />
            </button>
          </div>
        <% else %>
              ...
              <%= for item <- @cart do %>
                <div class="bg-white/5 border border-white/10 rounded-lg p-2 flex items-center gap-2">
                  <div class="flex-1 min-w-0">
                    ...
                    <p class="text-[10px] text-slate-400">KES <%= item.product.price %> × <%= item.quantity %></p>
                  </div>
                  <div class="flex items-center gap-1">
                    <button phx-click="update_quantity" phx-value-product_id={item.product.id} phx-value-delta="-1"
                      class="w-6 h-6 bg-slate-800 hover:bg-slate-700 rounded flex items-center justify-center">
                      <.icon name="hero-minus" class="w-3 h-3" />
                    </button>
                    <input id={"qty-input-#{item.product.id}"} type="number" min="1" value={item.quantity}
                      phx-blur="set_quantity" phx-value-product_id={item.product.id} name="quantity"
          ...
          <div class="px-3 py-3 border-t border-white/10 space-y-2 flex-shrink-0">
            ...
            <button phx-click="show_payment" disabled={@cart == []}
    ...
    <%= if @mpesa_modal_open do %>
      <div class="fixed inset-0 z-[60] flex items-center justify-center">
        <div phx-click="close_mpesa_modal" class="absolute inset-0 bg-black/60 backdrop-blur-sm"></div>
          ...
          <%= if @mpesa_pending do %>
            <div class="space-y-6 text-center py-4">
              ...
              <div>
                ...
                <p class="text-sm text-slate-400 mt-1">An STK Push has been sent to <strong><%= @mpesa_phone %></strong></p>
              </div>
              <button phx-click="close_mpesa_modal" class="px-6 py-2 bg-white/5 hover:bg-white/10 rounded-xl text-sm font-medium transition-all">Cancel</button>
            </div>
          <% else %>
            <%= if @mpesa_result && @mpesa_result.status == :failed do %>
              <div class="bg-red-500/10 border border-red-500/20 rounded-xl p-4 mb-6">
                <p class="text-sm text-red-400 text-center font-medium">STK Push failed or timed out. Please try again.</p>
              </div>
            <% end %>
            <form phx-submit="submit_mpesa" class="space-y-6">
              <div>
                ...
                <input name="mpesa[phone]" type="tel" required placeholder="07XXXXXXXX" class="w-full bg-slate-900 border border-white/10 rounded-xl py-3 px-4 text-white text-lg focus:ring-2 focus:ring-yellow-500/50 outline-none" />
              </div>
              <div class="flex items-center gap-3 justify-end">
                <button type="button" phx-click="close_mpesa_modal" class="px-6 py-3 bg-white/5 hover:bg-white/10 rounded-xl text-sm font-semibold transition-all">Cancel</button>
                <button type="submit" class="px-8 py-3 bg-yellow-500 hover:bg-yellow-400 text-black rounded-xl font-bold shadow-lg shadow-yellow-500/20 transition-all">Send STK Push</button>
              </div>
            </form>
          <% end %>
        </div>
    ...
    <%= if @card_modal_open do %>
      <div class="fixed inset-0 z-[60] flex items-center justify-center">
        <div phx-click="close_card_modal" class="absolute inset-0 bg-black/60 backdrop-blur-sm"></div>
        <div class="relative bg-[#0B0F1A] p-8 rounded-2xl w-full max-w-md border border-white/10 shadow-2xl">
          <h3 class="text-xl font-bold mb-6 flex items-center gap-2">
          ...
          <%= if @card_pending do %>
            <div class="space-y-6 text-center py-4">
              ...
              <button phx-click="close_card_modal" class="px-6 py-2 bg-white/5 hover:bg-white/10 rounded-xl text-sm font-medium transition-all">Cancel</button>
            </div>
          <% else %>
            <%= if @card_result && @card_result.status == :failed do %>
              <div class="bg-red-500/10 border border-red-500/20 rounded-xl p-4 mb-6">
                <p class="text-sm text-red-400 text-center font-medium">Transaction declined. Please check details or try another card.</p>
              </div>
            <% end %>
            <form phx-submit="submit_card" class="space-y-6">
              <div>
                ...
                <div class="relative">
                  ...
                  <input name="card[number]" type="text" required placeholder="4242 4242 4242 4242" class="w-full bg-slate-900 border border-white/10 rounded-xl py-3 pl-12 pr-4 text-white font-mono focus:ring-2 focus:ring-blue-500/50 outline-none" />
                </div>
              </div>
              <div class="grid grid-cols-2 gap-4">
                <div>
                  <label class="text-sm text-slate-400 block mb-2 font-medium">Expiry (MM/YY)</label>
                  <input name="card[expiry]" type="text" required placeholder="12/26" class="w-full bg-slate-900 border border-white/10 rounded-xl py-3 px-4 text-white font-mono focus:ring-2 focus:ring-blue-500/50 outline-none" />
                </div>
                <div>
                  <label class="text-sm text-slate-400 block mb-2 font-medium">CVV</label>
                  <input name="card[cvv]" type="text" required placeholder="123" class="w-full bg-slate-900 border border-white/10 rounded-xl py-3 px-4 text-white font-mono focus:ring-2 focus:ring-blue-500/50 outline-none" />
                </div>
              </div>
              <div>
                <label class="text-sm text-slate-400 block mb-2 font-medium">Cardholder Name</label>
                <input name="card[name]" type="text" required placeholder="Jane Mwangi" class="w-full bg-slate-900 border border-white/10 rounded-xl py-3 px-4 text-white focus:ring-2 focus:ring-blue-500/50 outline-none" />
              </div>
              <div class="flex items-center gap-3 justify-end pt-4">
                <button type="button" phx-click="close_card_modal" class="px-6 py-3 bg-white/5 hover:bg-white/10 rounded-xl text-sm font-semibold transition-all">Cancel</button>
                <button type="submit" class="px-8 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold shadow-lg shadow-blue-500/20 transition-all">Process Payment</button>
              </div>
            </form>
          <% end %>
        </div>
