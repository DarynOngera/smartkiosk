defmodule SmartKioskWeb.UserRegistrationLive do
  ...
  def render(assigns) do
    ~H"""
    <Layouts.app flash={@flash} current_path="/register">
            ...
            <div class="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-2xl">
              ...
              >
                ...
                <div class="space-y-2">
                  ...
                  <%= for error <- @form[:full_name].errors do %>
                ...
                <%= if @reg_type == "shop" do %>
                  ...
                  <div class="space-y-2 animate-fade-in">
                    ...
                    <%= for error <- @form[:shop_category].errors do %>
                      <p class="mt-1 text-sm text-rose-300"><%= translate_error(error) %></p>
                    <% end %>
                  </div>
                <% end %>
  ...
  def handle_event("validate", params, socket) do
    ...
    changeset = registration_changeset(socket.assigns.reg_type, registration_params)
    ...
    if socket.assigns.reg_type == "shop" do
      ...
      case Accounts.register_shop_owner(shop_attrs, user_attrs) do
        ...
        {:error, %Ecto.Changeset{} = changeset} ->
          changeset =
            registration_changeset("shop", registration_params)
            |> merge_registration_errors(changeset)

          {:noreply,
           assign(socket, :form, to_form(changeset, as: :registration, action: :insert))}
      end
    else
  ...
  defp registration_changeset("customer", params) do
    Accounts.change_user_registration(%User{}, params)
  end

  defp registration_changeset(_reg_type, params) do
  ...
  defp merge_registration_errors(registration_changeset, %Ecto.Changeset{} = error_changeset) do
    Enum.reduce(error_changeset.errors, registration_changeset, fn {field, {message, opts}},
                                                                   changeset ->
